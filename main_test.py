...
# Завантаження даних
auditoriums = read_auditoriums('auditoriums1.csv')
groups = read_groups('groups1.csv')
lecturers = read_lecturers('lecturers1.csv')
subjects = read_subjects('subjects1.csv')
...
if missing_subjects:
    print(f"Warning: There are no teachers for the following subjects: {', '.join(missing_subjects)}")
...
def is_conflict(lesson, time_slot, timetable):
    for existing_lesson in timetable[time_slot]:
        ...
        if lesson.auditorium and existing_lesson.auditorium and existing_lesson.auditorium.id == lesson.auditorium.id:
            ...
            if not (
                lesson.type == 'Lecture' and
                existing_lesson.type == 'Lecture' and
    ...
    for _ in range(POPULATION_SIZE):
        ...
        for subject in subjects:
            ...
            for _ in range(lectures_total):
                lesson = Lesson(subject, 'Lecture', group)
            ...
            if subject.requires_subgroups and group.subgroups:
                for subgroup in group.subgroups:
                    for _ in range(pract_total // len(group.subgroups)):
                        lesson = Lesson(subject, 'Practice', group, subgroup)
                        possible_lecturers = get_possible_lecturers(lesson)
                        if not possible_lecturers:
                            continue
                        lecturer = random.choice(possible_lecturers)
                ...
                for _ in range(pract_total):
                    lesson = Lesson(subject, 'Practice', group)
                    possible_lecturers = get_possible_lecturers(lesson)
                    if not possible_lecturers:
                        continue
                    lecturer = random.choice(possible_lecturers)
